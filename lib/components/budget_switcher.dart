  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<BudgetsBloc, BudgetsState>(listener: (context, state) {
      ...
      if (state is BudgetsLoadSuccess) {
        if (controller.budget == null)
          controller.set(context, state.budgets[0]);
        return BudgetSwitcherLoaded(
            controller: controller, budgets: state.budgets);
      }
      if (state is BudgetsLoadFailure)
        return BudgetSwitcherLoadFailed(message: state.error);
      return BudgetSwitcherLoading();
    }));
  }
}

class BudgetSwitcherLoadFailed extends StatelessWidget {
  final String message;

  BudgetSwitcherLoadFailed({@required this.message});
...
class BudgetSwitcherLoading extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
...
class BudgetSwitcherLoaded extends StatefulWidget {
  final BudgetControllerSingleton controller;
  final List<Budget> budgets;

  BudgetSwitcherLoaded({@required this.controller, @required this.budgets});

  @override
  BudgetSwitcherLoadedState createState() => BudgetSwitcherLoadedState();
}

class BudgetSwitcherLoadedState extends State<BudgetSwitcherLoaded> {
  @override
  Widget build(BuildContext context) {
